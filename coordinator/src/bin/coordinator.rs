...
const EVENT_LOOP_COOLDOWN: Duration = Duration::from_millis(30000);
...
async fn check_nodes(ctx: SharedState, client: hyper::Client<HttpConnector>) {
    ...
    let to_socket_addrs =  server_nodes.to_socket_addrs();
    log::info!("to_socket_addrs {:?}",to_socket_addrs);
    let mut addrs = to_socket_addrs
        .unwrap()
